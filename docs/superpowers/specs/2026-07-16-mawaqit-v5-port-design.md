...
- Plugin at repo-root `mawaqit/`; `id="aalbayaty/mawaqit"`, `author="aalbayaty"`.
- Do **not** hand-edit `catalog.toml` (auto-generated by the repo's Action).
...
1. **Scaffold + manifest + i18n** — `plugin.toml` (all settings), `translations/en.json`,
